...
const handleGenerateAllBills = async () => {
  ...
  failedCustomersList.push(
    `${currentCustomer.customerName} - ${
      ...
      setProgress(
        Math.round(
          (current / activeCustomers.length) * 100
        )
      );
...
try {
  ...
  alert(
    error.response?.data?.message ||
      "Failed to send bills to WhatsApp."
  );
}
...
<div className="mt-8">
  ...
  <div className="flex flex-wrap gap-4">
    ...
    <button
      ...
      className="bg-red-600 hover:bg-red-700 disabled:opacity-60 disabled:cursor-not-allowed text-white px-8 py-3 rounded-xl font-bold"
    >
      {bulkGenerating
        ? "Generating..."
        : "Generate All Bills"}
